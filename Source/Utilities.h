...
namespace EngineHelpers
{
    ...
    void browseForAudioFile (std::function<void (const File&)> fileChosenCallback, String fileFormats = "*.wav;*.mp3;*.aif")
    {
        auto fc = std::make_shared<FileChooser> ("Please select a file to load...",
                                                 juce::File::getCurrentWorkingDirectory(),
                                                 fileFormats);

        fc->launchAsync (FileBrowserComponent::openMode + FileBrowserComponent::canSelectFiles,
                         [fc, callback = std::move (fileChosenCallback)] (const FileChooser&)
                         {
                             const auto f = fc->getResult();
